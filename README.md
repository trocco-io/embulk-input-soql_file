...
- Using username and password authentication (`auth_method: user_password`):
  ...
  - Note: `user_password` relies on the SOAP `login()` call, which is not available with API version 65.0 or later (it fails with `UNSUPPORTED_API_VERSION`). Set `api_version` to `64.0` or lower when using this auth method (the plugin rejects `user_password` with `api_version` 65.0 or later as a configuration error). Salesforce plans to retire SOAP `login()` entirely around Summer '27, so consider migrating to `oauth`. See the [Salesforce release update](https://help.salesforce.com/s/articleView?id=release-notes.rn_api_upcoming_retirement_258rn.htm&language=en_US&release=258&type=5).
